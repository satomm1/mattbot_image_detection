...
def callback(data):
    ...
    input_data = np.transpose(input_data, axes=[0,3,1,2])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model', default="cones.tflite", help='Path to the TFLite model.')
    parser.add_argument('--topic', default="/camera/color/image_raw", help='Path to the labels file.')
    parser.add_argument('--score_threshold', type=float, default=0.25, help='Detection threshold.')
    parser.add_argument('--labels', default="cones_labels.txt", help='Path to the labels file.')
    ...
    labeled_image_publisher = rospy.Publisher('labeled_image', RosImage, queue_size=1)
