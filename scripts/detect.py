...
def callback(data):
    ...
    labeled_image_publisher.publish(img)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Detect objects in a video stream.')
    parser.add_argument('--model', default="yolov8n_int8.tflite", help='Path to the TFLite model.')
    ...
    image_subscriber = rospy.Subscriber(args.topic, RosImage, callback)
    rospy.spin()